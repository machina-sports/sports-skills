"""Sportradar NFL event document -> ``canonical-observation/1``.

**Read the source label before reading anything else.** No Sportradar NFL payload
is checked into this repository — not a response, not a sanitized example. The
only evidence is ``tools/iptc/fixtures/baseline/sportradar-nfl-event.json``,
which PR 1 hand-authored from the literal key set of
``iptc-sportradar-event-nfl-mapping``. So this adapter's input is a **legacy
...
is a claim to redistribute Sportradar data or a claim about Sportradar's real NFL
coverage.**
...
``schema:sportName`` **says a league, and the sport is asserted separately.**
    The mapping hardcodes ``"nfl"`` there. A league is not a sport, so this
    adapter treats the value as the discriminator it actually is —
    :func:`_check_sport` refuses any other value — and states the sport itself as
    :data:`SPORT_MEDTOP`, ``20000823`` (American football), a ``skos:Concept`` in
    the pinned mediatopic scheme. The check matters more here than for the soccer
    feeds: the NFL and MLB mappings share every URN stem, so a document read by
...
``sport:status`` **is mapped, and an unmapped value raises.**
    ...
    ``sport:matchStatus``, so the two are copies of one field. Only
    ``sport:status`` is read: two places for one answer is two places for them to
    disagree. :data:`EVENT_STATUS_BY_CODE` holds **three** codes, and the
    narrowness is the point — the mapping rewrites ``created`` and ``scheduled``
    to ``not_started`` and ``inprogress`` to ``live``, and the checked-in shape
    carries ``closed``. That is the whole of this repository's evidence for the
    vocabulary. ``complete``, ``interrupted``, ``halftime`` and the rest of
    Sportradar's real game-status enum are deliberately absent rather than
    guessed: ``observation.event.status`` is required, no default is defensible,
    and a plausible neighbour that validates is far more expensive than a
    :class:`ValueError` naming the code.
...
fields.**
    ...
    literals in the mapping expression: the schedule payload it consumes carries
    no competition entity and no season entity at all. The event, venue and team
...
``sport:score.sport:halfTime`` **is not a clock and not a period.**
    It is the first Sportradar period object, verbatim, keys and all
    (``period_type``, ``number``, ``sequence``, ``home_points``,
    ``away_points``) — a score *for* one period, not a reading of how far into the
    game play had reached. Emitting ``event.clock`` from it would tell a consumer
    a closed game was in its first quarter. It survives in ``raw``, which is also
    where its provider key names stay: forwarding them was the provider-leak
    defect the baseline row is fixtured for.

**A 24-17 scoreline produces no winner.**
    The source states no winner and no outcome. Comparing two numbers is
    inference, and ``event.result`` is optional at the ``core`` tier, so absence
    costs nothing except the false claim.

**No phase, no competition type, no statistics, no roster.**
    The NFL genuinely has a regular season and a post-season, and this document
    says which one this game is in nowhere at all, so ``spct:season-regular``
    would be a guess. The connector has separate statistics mappings; joining one
    to this document would attach numbers to a game nothing here says they belong
    to. ``sport:abbreviation`` is not an official term and ``TeamShape`` is
    ``sh:closed`` admitting only ``rdfs:label``, so the alias stays in ``raw``
    too.
...
ADAPTER_NAME = "tools.iptc.canonical.adapters.sportradar_nfl"
...
#: Scoped to the NFL connector rather than to "sportradar", because Sportradar
#: publishes a separate feed per sport and each gets its own adapter. It matters
#: doubly here: the legacy NFL and MLB mappings both mint
#: ``urn:sportradar:team:<id>`` with no sport in the stem, so their identifier
#: spaces collide in the old model and one shared namespace would carry that
#: collision into the crosswalk.
PROVIDER_NAMESPACE = "sportradar-nfl"
...
SOURCE_MAPPING = "iptc-sportradar-event-nfl-mapping"
...
#: IPTC ``medtop`` for American football, and the ``event_view`` sport key.
#: 20000823 is a ``skos:Concept`` in the pinned ``vocabularies/mediatopic.ttl``,
#: which is what makes it checkable rather than plausible; layer 4 checks it.
SPORT_MEDTOP = "20000823"
...
#: this is the collision with the MLB mapping that :data:`PROVIDER_NAMESPACE`
...
#: docstring is checkable and a future reader adding a fourth constant has to
...
#: addresses its competition as ``nfl`` and its season as ``2025``, which this
#: repository has no evidence for and the mapping's own source contradicts.
#: Naming the constants without marking them was the state before A16: the
#: limitation was recorded in prose and the machine-readable field said the
#: opposite.
...
#: ``speventstatus:`` NewsCode. Three entries, and the narrowness is the point;
#: see this module's docstring. A test asserts the keys are exactly the codes this
#: repository has evidence for, and that every value is a key of that table.
...
def _event_status(document):
    ...
    if code is None:
        raise ValueError(
            "sportradar nfl document has no sport:status, so its canonical event "
            "status cannot be determined; no observation was produced"
        )
    if code not in EVENT_STATUS_BY_CODE:
        raise ValueError(
            "sportradar nfl sport:status '{0}' has no canonical event status in "
...
def _check_sport(document):
    ...
    if stated is not None and stated != SPORT_NAME:
        raise ValueError(
            "sportradar nfl document states sport '{0}', but this adapter asserts "
            "medtop {1} (American football, '{2}'). Use the adapter for that "
...
def _participant(competitor, score_block):
    ...
    # No outcome: the source states no winner, and 24-17 is a scoreline rather
    # than a result. No statistics: the event mapping emits none.
...
def _event(document, provider_id, status, label):
    ...
    event["status"] = status
    # No clock: sport:halfTime is a score for one period, not a reading. No
    # attendance and no end time: the event mapping emits neither.
    return event


def to_observation(payload, *, observed_at):
    """One Sportradar NFL event document as a ``canonical-observation/1``.
    ...
    if event_id is None:
        raise ValueError(
            "sportradar nfl document has no {0} @id, so there is nothing to mint "
    ...
    # No phase: the source states no stage, round or week.
    ...
    # declined to map — the raw Sportradar period object under sport:halfTime
    # with its provider key names, the team abbreviations and the mapping's
    # duplicate sport:matchStatus — is readable here, which is what makes "we
    # omitted it" checkable.
