...
- Coverage: all ESPN-backed modules (NFL, NBA, WNBA, NHL, MLB, college, golf, tennis, cricket via ESPN), football (ESPN, Understat, FPL, Transfermarkt), Kalshi and Polymarket public reads, nflverse (schedules, weekly rosters, player and team stats, play-by-play) and FastF1 (schedules, session results, laps). ProphetX, esports, volleyball, metadata, XCTF, Cricsheet, openfootball and RSS news are not yet covered and still reach the network in replay mode. Polymarket trading (`polymarket-trading`) is never recorded.
